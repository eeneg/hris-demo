...
class LeaveApprovedListener
{
    ...
    public function handle(LeaveProcessed $event)
    {
        ...
        $leave = [
            ...
            'vl_balance' => $data->vacation_balance - $data->vacation_less,
            'vl_withoutpay' => $is_sl != true ? $data->days_without_pay : 0,
            'sl_earned' => 0,
            'sl_withpay' => $is_sl ? $data->days_with_pay : 0,
            'sl_balance' => $data->sick_balance - $data->sick_less,
        ...
        $s = LeaveCredit::updateOrCreate(['personal_information_id' => $this->request->personal_information_id, 'leave_type_id' => $sl->id], ['balance' => $data->sick_balance - $data->sick_less]);
        $d = LeaveCredit::updateOrCreate(['personal_information_id' => $this->request->personal_information_id, 'leave_type_id' => $vl->id], ['balance' => $data->vacation_balance - $data->vacation_less]);
