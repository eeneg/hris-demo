                    ...
                    <div class="card-body">
                        <form-wizard ref="wizard" :start-index="0" shape="tab" title="" subtitle="" color="#3490dc">
                            ...
                            <tab-content title="Personal Information" icon="ti-user">
                                ...
                                <div class="form-group" style="display: flex; margin-top: 50px;">
                                    <div class="col-md-12">
                                        <label for="" style="line-height: 10px; color:green">RELIGION</label>
                                        <hr/>
                                    </div>
                                </div>
                                ...
                                <div class="form-group" style="display:flex">
                                    ...
                                    <div class="col-md-6" v-if="form.religion == 'Others'">
                                        <div>
                    ...
                    if((value && (typeof(value) === "object")) && Array.isArray(value))
                    {
                        if(value.length >= 0)
                        {
                            value.forEach(function(data, index){
                                if(Object.keys(data).length > 0 && !(Object.keys(data).length == 1 && Object.keys(data)[0] == 'orderNo'))
                                {
                                    if(data['id'])
                                    {
                                        var old = _.find(oldValue[key], {id: data['id']})
                                        for(var field in data)
                                        {
                                            if(data[field] != old[field] && data[field] !== null)
                                            {
                                                vm.edits.push({model_id: data['id'], model: key, field: field, oldValue: old[field], newValue: data[field], status: 'PENDING'})
                                            }
                                        }
                                    }
                                    if(!data['id'] && field !='createdAt' && field !='updatedAt' && data[field] !== null){
                                        for(var field in data)
                                        {
